...
with DHT11.
The data can be save to server Z:\test\
...
byte ip[] = { 192,168,190,188};      //The Arduino device IP address
byte subnet[] = { 255,255,255,0};    //The Arduino device subnet
byte gateway[] = { 192,168,190,43};   //The Arduino device gateway
...
void loop()
{
    ...
    if (!connected)
    {
          ...
          connected = true;
          delay(2000);
          DHT.read22(dht_dpin);      //change the "DHT.read11" to "DHT.read11" if used DHT11 sensor
         ...
         if( count == 5){
           ...
           delay(4000);
           String gotcha = "GET /test/temp.php?ip=192.168.190.188&temp=";  //set the ip address of Tempsense used
           gotcha += tempchr;
           gotcha += "&humid=";
